...
denovolyzeByClass(dnm.genes=autismDeNovos$gene,
                  dnm.classes=autismDeNovos$class,
                  nsamples=1078)
```
...
There is another important option here.  The expected number of genes containing >1 hit is obtained by permutation: Given n *de novo* variants, how many genes contain >1 de novo?  There are two options for selecting n: by default it is derived from your data: e.g. in the example above autismDeNovos contains `r sum(autismDeNovos$class %in% c("frameshift","non","splice"))` lof variants, so this is the number used in the permutation. This is controlled by the default parameter `expectedDNMs="actual"`
``` {r}
